...
from utils.settings import route, result_save_path, fakedata_save_path, Sorts
...
for sort in Sorts:  # 每一个种类单独训练
    # 初始化网络每一层的参数
    d.apply(init_weights), g.apply(init_weights)

    # #恢复训练
    # g=torch.load(os.path.join(result_save_path,sort,sort+'_g__last.pth'))
    # d=torch.load(os.path.join(result_save_path,sort,sort+'_d__last.pth'))

    # 初始化训练数据读取器
    train_dataset = LoadData(os.path.join(route, sort), number_of_channels=g_d_nc)  # dataset
    train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, drop_last=drop_last)  # dataloader
    ...
    for epoch in tqdm(range(0, num_epoch+1), desc='epoch'):  # 迭代num_epoch个epoch
        ...
        if epoch % output_loss_Interval_ratio == 0:
            print('\nsort:{} Epoch[{}/{}],d_loss:{:.6f},g_loss:{:.6f} '
                  'D real: {:.6f},D fake: {:.6f}'.format(
                      sort, epoch, num_epoch,
        ...
        if not os.path.exists(os.path.join(result_save_path, sort)):
            os.mkdir(os.path.join(result_save_path, sort))

        # 保存生成的fake图片，间隔save_model_Interval_ratio个epoch保存一次
        if epoch % save_model_Interval_ratio == 0:
            save_image(fake_img, os.path.join(result_save_path, sort,
                                              sort+'_fake_epoch'+str(epoch)+'.jpg'))
        ...
        plt.ylabel("loss")
        plt.legend()
        plt.savefig(os.path.join(result_save_path, sort, 'loss.jpg'))
        plt.clf()
